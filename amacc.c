...
void expr(int lev)
{
    ...
    case And: // "&", take the address operation
         ...
         */
        next(); expr(Inc);
        if (*n == Load) n = n+2; else { fatal("bad address-of\n");}
        ty += PTR;
        break;
    ...
    while (tk >= lev) {
        ...
        case MulAssign:
            ...
            int *l = n;
            expr(Mul);
            *--n = (int)l; *--n = Add + (otk - AddAssign); *--n = (int)(b+2); *--n = ty = t; *--n = Assign; 
            ty = INT; 
            break;
            ...
            if (*n == Num && *b == Num) 
                ...
                *--n = (int)b; *--n = Sub; 
                if (t == ty && sz > 1) {
                    switch (sz) {
                        case 4: 
                        *--n = 2; *--n = Num; --n; *n = (int)(n+3);  *--n = Shr;
                        break;
                    default: 
                        *--n = sz; *--n = Num; --n; *n = (int)(n+3); *--n = Sub;
                        break;
                    }
                }
            }
        ...
        case Mul:
            next(); 
            expr(Inc); 
            if (*n==Num && *b==Num) n[1] = b[1] * n[1]; else { *--n = (int)b; *--n = Mul; } 
            ty = INT;    
            break;
        case Div: 
            next(); 
            expr(Inc); 
            if (*n==Num && *b==Num) n[1] = b[1] / n[1]; else { *--n = (int)b; *--n = Div; } 
                ty = INT;
            break;
        case Mod:
            next(); expr(Inc); 
            if (*n==Num && *b==Num) n[1] = b[1] % n[1]; else { *--n = (int)b; *--n = Mod; } 
            ty = INT; 
            break;
        case Inc:
        case Dec:
        ...
        case Brak:
            ...
            if (sz > 1) { if (*n == Num) n[1] = n[1] * sz; else { *--n = sz; *--n = Num; --n; *n = (int)(n+3); *--n = Mul; } }
...
void stmt()
{
    ...
    case For:
        ...
        *--n = ';';
        expr(Assign);
        while (tk == ',') { e = n; next(); expr(Assign); *-- n = (int)e; *--n = '{'; }
        d = n;
        if (tk == ';') next();
        else fatal("semicolon expected");
        *--n = ';';
  ...
  i = *n;
    ...
    switch(i){
        ...
        case Func:
            ...
            k = 0;
            // how many parameters
            while(b){ ++k; if(!(int*)*b){ break; };  b = (int*)*b; } 
            int j = 0;
            int *pp = malloc(sizeof(int*)*k);
            b = c;
            while(b){ pp[j] =(int)b;  if(!(int*)*b){ break; };  b = (int*)*b; ++j; }
            while (j >= 0) { gen(b+1);  *++e = PSH; --j; b = (int*)pp[j]; }
            free(pp);
        ...
        case Break:
            *++e = JMP; *++e = (int) brks; brks = e;
            break;
        case Default:
            if(cas) *cas = (int)(e+1); gen((int *)n[1]); break;
